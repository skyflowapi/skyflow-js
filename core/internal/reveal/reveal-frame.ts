...
*/
// Shared reveal-frame base (iframe-only). Holds the token-based reveal DOM/bus
// machinery common to both SDKs. It is boundary-clean (@core imports only) and
// carries NO file-render path — privacyDB's file-render feature (renderFile /
// getFileURLFromVaultBySkyflowIDComposable / formatForRenderClient) lives in the
// skyflow-js subclass and is wired in through the two protected hooks below, so
// it never reaches flowDB's bundle. flowDB, being token-only, re-exports this
// base unchanged.
...
import {
  Context, MessageType, RedactionType,
} from '@core/types';
import {
...
class RevealFrame {
  ...
  protected dataElememt: HTMLSpanElement;
  ...
  protected inputStyles!: object;

  protected labelStyles!: object;

  protected errorTextStyles!: object;

  protected revealedValue!: string;

  protected context: Context;

  protected domCopy?: HTMLImageElement;

  protected isRevealCalled?: boolean;

  protected skyflowContainerId: string = '';

  protected composableContainer: Boolean = false;
  ...
  static init() {
    ...
    RevealFrame.revealFrame = new this(parsedRecord.record,
  ...
  constructor(record, context: Context, id: string, rootDiv?: HTMLDivElement) {
    ...
    this.registerRenderFileResponseListener();
    ...
    window.addEventListener('message', (event) => {
      this.handleRevealCallRequest(event);
  ...
  // Hook: privacyDB registers its RENDER_FILE_RESPONSE_READY bus listener here.
  // Token-only flowDB leaves it empty, so the file-render path never wires up.
  // eslint-disable-next-line class-methods-use-this
  protected registerRenderFileResponseListener(): void {}

  // Hook: privacyDB handles RENDER_FILE reveal-call requests (message events)
  // here. Token-only flowDB no-ops, keeping the file-render surface out.
  ...
  protected setRevealError(errorText: string) {
