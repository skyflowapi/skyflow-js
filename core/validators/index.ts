...
*/
// Variant-neutral validators shared by privacyDB and flowDB. Two groups:
//  1. Pure validators (card / Luhn / expiry-format / URL / regex) — no deps.
//  2. "Shape" validators for the variant-neutral request/response inputs
//     (insert / update / get / getById / detokenize / delete / upsert /
//     composable-container / input-format / init-config / expiry-date). These
//     use SkyflowError + @core/types input types — all core-reachable — so they
//     live here too. Each package's src/utils/validators re-binds this set and
//     keeps only genuinely variant-specific rules local (reveal-input, which
//     differs privacyDB↔flowDB, and collect-input, which needs package logging).
import {
...
import { appendZeroToOne } from '@core/helpers';
import {
  IInsertRecordInput,
  IUpdateRequest,
  IUpdateOptions,
