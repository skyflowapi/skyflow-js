...
describe('test formatOptions function with format and translation', () => {
    ...
    test('should return preserveFileName true for MULTI_FILE_INPUT when not provided in options',()=>{
        const formattedOptions = formatOptions(ElementType.MULTI_FILE_INPUT,{required:true},LogLevel.ERROR);
        expect(formattedOptions).toEqual({required:true,preserveFileName:true});
    });

    test('should keep preserveFileName false for MULTI_FILE_INPUT when provided as false',()=>{
        const formattedOptions = formatOptions(ElementType.MULTI_FILE_INPUT,{required:true,preserveFileName:false},LogLevel.ERROR);
