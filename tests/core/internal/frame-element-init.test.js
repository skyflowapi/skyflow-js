...
describe('FrameElementInit Additional Test Cases', () => {
    ...
    // test('should handle multi file upload message event with success', async () => {
    //         const composableElement = {
    //             ...element,
    //             rows: [{
    //                 elements: [{
    ...
    //         // Mock successful network requests for each file
    //         const mockRequest = jest.fn().mockResolvedValue({ skyflow_id: 'id1', file: 'test-file-1.txt' });
    //         Client.prototype.request = mockRequest;
    //         Client.prototype.config = { vaultURL: 'https://vaulturl.com', vaultID: 'vault123' };

    //         FrameElementInit.startFrameElement();
    //         postMessageSpy.mockClear();

    //         // Find MULTI_FILE_INPUT form element and inject FileList state
    //         const multiFileElement = FrameElementInit.frameEle.iframeFormList.find(
    //             (f) => f.iFrameName === 'element:MULTI_FILE_INPUT:123'
    //         );
    //         expect(multiFileElement).toBeTruthy();

    //         if (multiFileElement) {
    //             Object.defineProperty(multiFileElement, 'state', {
    //                 value: {
    //                     value: mockFileList, // emulate user having selected files
    //                     name: 'file_uploads',
    //                     isRequired: false,
    //                     isValid: true,
    //                     isEmpty: false,
    ...
    //         windowSpy.mockImplementation(() => windowMock); // keep same window for handler

    //         if (!messageHandler) throw new Error('messageHandler missing');
    ...
    //                 type: `${ELEMENT_EVENTS_TO_IFRAME.MULTIPLE_UPLOAD_FILES_RESPONSE}:element:MULTI_FILE_INPUT:123`,
    //                 data: expect.objectContaining({
    //                     fileUploadResponse: expect.arrayContaining([
    //                         expect.objectContaining({ skyflow_id: 'id1' }),
    //                     ]),
    //                 }),
    //             }),
    //             'http://localhost.com'
    ...
    //         // Ensure request called once per file
    //         expect(mockRequest).toHaveBeenCalledTimes(mockFileList.length);
    //     });
  });
