...
export const validateThreeDSInput = (threeDSInput: IThreeDSInput) => {
  if (
    ...
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_OBJECT_IN_3DS);
  }
  ...
  if (!threeDSInput.cardDetails.cardNumber) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_CARD_NUMBER_IN_3DS);
  }
  if (
    !(
    ...
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_CARD_NUMBER_IN_3DS);
  }

  if (!threeDSInput.cardDetails.cardHolderName) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_CARDHOLDER_NAME_IN_3DS);
  }
  if (
    !(
    ...
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_CARDHOLDER_NAME_IN_3DS);
  }
  if (!threeDSInput.cardDetails.cardExpiry) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_CARD_EXPIRY_IN_3DS);
  }
  if (
    !(
    ...
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_CARD_EXPIRY_IN_3DS);
  }

  if (!threeDSInput.cardDetails.schemeID) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_SCHEME_ID_IN_3DS);
  }
  if (!(typeof threeDSInput.cardDetails.schemeID === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_SCHEME_ID_IN_3DS);
  }

  if (!threeDSInput.config.acquirerDetails.acquirerBIN) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_ACQUIRER_BIN_IN_3DS);
  }
  if (!(typeof threeDSInput.config.acquirerDetails.acquirerBIN === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_ACQUIRER_BIN_IN_3DS);
  }

  if (!threeDSInput.config.acquirerDetails.acquirerMerchantID) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_ACQUIRER_MERCHANT_ID_IN_3DS);
  }
  if (!(typeof threeDSInput.config.acquirerDetails.acquirerMerchantID === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_ACQUIRER_MERCHANT_ID_IN_3DS);
  }

  if (threeDSInput.config.acquirerDetails.acquirerID && !(typeof threeDSInput.config.acquirerDetails.acquirerID === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_ACQUIRER_ID_IN_3DS);
  }

  if (!threeDSInput.config.merchantDetails.mcc) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_MERCHANT_MCC_IN_3DS);
  }
  if (!(typeof threeDSInput.config.merchantDetails.mcc === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_MERCHANT_MCC_IN_3DS);
  }

  if (!threeDSInput.config.merchantDetails.merchantName) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_MERCHANT_NAME_IN_3DS);
  }
  if (!(typeof threeDSInput.config.merchantDetails.merchantName === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_MERCHANT_NAME_IN_3DS);
  }

  if (!threeDSInput.config.merchantDetails.merchantUrl) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_MERCHANT_URL_IN_3DS);
  }
  if (!(typeof threeDSInput.config.merchantDetails.merchantUrl === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_MERCHANT_URL_IN_3DS);
  }

  if (!threeDSInput.config.merchantDetails.merchantCountryCode) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_MERCHANT_COUNTRY_IN_3DS);
  }
  if (!(typeof threeDSInput.config.merchantDetails.merchantCountryCode === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_MERCHANT_COUNTRY_IN_3DS);
  }

  if (!threeDSInput.config.threeDSRequestorName) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_REQUESTOR_NAME_IN_3DS);
  }
  if (!(typeof threeDSInput.config.threeDSRequestorName === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_REQUESTOR_NAME_IN_3DS);
  }

  if (!threeDSInput.config.threeDSRequestorId) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_REQUESTOR_ID_IN_3DS);
  }
  if (!(typeof threeDSInput.config.threeDSRequestorId === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_REQUESTOR_ID_IN_3DS);
  }

  if (!threeDSInput.config.threeDSRequestorFinalAuthRespURL) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_REQUESTOR_URL_IN_3DS);
  }
  if (!(typeof threeDSInput.config.threeDSRequestorFinalAuthRespURL === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_REQUESTOR_URL_IN_3DS);
  }

  if (threeDSInput.config.threeDSServerTransId && !(typeof threeDSInput.config.threeDSServerTransId === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_REQUESTOR_TRANS_ID_IN_3DS);
  }

  if (threeDSInput.config.preferredProtocolVersion && !(typeof threeDSInput.config.preferredProtocolVersion === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_PROTOCOL_VERSION_IN_3DS);
  }

  if (!threeDSInput.amountDetails.amount) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_AMOUNT_IN_3DS);
  }
  if (!(typeof threeDSInput.amountDetails.amount === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_AMOUNT_IN_3DS);
  }

  if (!threeDSInput.amountDetails.purchaseCurrency) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.MISSING_PURCHASE_CURRENCY_IN_3DS);
  }
  if (!(typeof threeDSInput.amountDetails.purchaseCurrency === 'string')) {
    throw new SkyflowError(SKYFLOW_ERROR_CODE.INVALID_PURCHASE_CURRENCY_IN_3DS);
  }
