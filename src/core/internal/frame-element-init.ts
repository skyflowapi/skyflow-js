...
export default class FrameElementInit {
  ...
  uploadFiles = (fileElement, clientConfig) => {
    ...
    const {
      state, tableName, skyflowID, onFocusChange, preserveFileName, returnFileMetadata
    } = fileElement;
    ...
    formData.append('returnFileMetadata', String(returnFileMetadata));
  ...
  (fileElement: IFrameFormElement, clientConfig, metaData) => new Promise((rootResolve, rootReject) => {
    ...
    const { state, tableName, onFocusChange, preserveFileName, returnFileMetadata } = fileElement;
    if (state.isRequired) {
      onFocusChange(false);
    }
